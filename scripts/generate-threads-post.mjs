...
async function generateThreadsContent(article, relatedArticles) {
  ...
  if (relatedArticles.length > 0) {
    ...
    relatedSection = `
## 過去7日間の関連記事
以下の過去記事を踏まえ、時系列的な文脈（「前にお話しした〇〇と合わせて考えると...」「以前から話題の〇〇がさらに進化して...」など）を自然に盛り込んでください。「先週」「昨日」など具体的な日付表現は避け、「前に」「以前」を使ってください。
...
`;
  ...
  const reply2Patterns = [
    ...
    '【活用×歴史ひとこと】活用例がメインだが、解説の流れの中で歴史のたとえを1文だけサラッと添える。「産業革命のときと同じで、使いこなした人が勝つんですよね」程度。歴史で1段落使わない。',
  ...
  const reply3Patterns = [
    '【締め×実用まとめ】今後の見通しを「読者が明日からできること」に落とし込んで締める。実用的で前向き。',
    '【締め×ひとこと予想】「半年後にはこれが当たり前になってるかも」くらいの軽い未来予想を1文で。重くしない。',
    '【締め×歴史ひとこと】「黒船のときと同じで、先に動いた人が勝つんですよね」程度の1文を添えるだけ。メインは今後の展望。',
    '【締め×素直な感想】哲学や歴史ではなく、ヒナキラの素直な感想で締める。「正直ワクワクが止まらない」「こういう時代に発信できて幸せだなと思う」など。',
  ];
...
- 最後の投稿は自然体で締める。オチや自虐ネタは毎回入れない（入れるとAI感が出る）。入れるなら3回に1回くらい、本当に自然にハマるときだけ

## ヒナキラの独自視点（ブランディング）
投稿のメインは「わかりやすい解説」と「読者にとっての実用的な価値」です。
以下の要素はあくまで**スパイス（添え物）**として、1〜2文でサラッと入れる程度にとどめてください。主役にしない。

【スパイスA：歴史の一言】
- 解説の流れの中で、歴史的なたとえを1文だけサラッと添える程度（例：「黒船来航のときと同じで、最初に動いた人が勝つんですよね」）
- 歴史だけで1段落使うのはNG。あくまで解説の補足として自然に差し込む
- 信長だけに偏らない。戦国・幕末・産業革命・インターネット黎明期など幅広く
- 毎回入れなくてもOK。自然にハマるときだけ

【スパイスB：自分ごと化】
- ヒナキラ自身の日常や仕事に引きつけた一言を添える（例：「僕もさっそく試してみたけど…」「正直、最初は『自分には関係ない』と思ってました」）
- 読者が「自分にも関係ある」と思える翻訳をする
...
- リプライ3：穏やかに締める。押し売り感のない自然なCTA。オチは無理に入れない
...

{
  ...
  "replies": [
    ...
    "リプライ3（300〜480文字）：今後の見通しで締めくくり + 最後に軽いCTA。${todayReply3} 本文の解説をしっかり締めた後、最後の2〜3行でCTAをコンパクトに。CTAの導入は『この記事の詳細はメルマガから見られるので、メルマガ登録もどうぞ！』のように、解説の続き→メルマガという自然な流れで繋げる。例：『もっと詳しくはメルマガで毎日お届けしてます。プロフィールのリンクから登録できます！』くらいの軽さ。フォローのお願いもサラッと。特典は入れても1個、入れなくてもOK。CTAが投稿の主役にならないこと。"
  ]
}
...
- フォローのお願いをサラッと入れる（例：「フォローしてもらえると嬉しいです」「よかったらフォローお願いします」程度。押し売りにならない一言で）
- CTAへの導入は「詳細はメルマガから見られるので、メルマガ登録もどうぞ！」のように、解説の続きが読めるという価値でメルマガに繋げる
- メルマガは「毎日AIニュースをギュッとまとめて配信しています」くらいのシンプルな一言 + 「プロフィールのリンクから登録できます」と案内
- 特典は以下の中から1個だけサラッと触れる（全部入れない。毎回違うものを選ぶ）：
  ・図解生成ツール（AIで図解が作れるツール）
  ・GPTsを作れるようになる動画（自分だけのAIが作れるようになる）
  ・オプチャ（オープンチャット）への秘密の入口（ヒナキラの思考やAI情報をリアルタイムで覗ける閲覧専用の場所）
  ・そのほかシークレット特典
- CTA全体を2〜3文に収める。長くしない。本文の解説がメインであることを忘れない
